...
public class PopMessageProcessor implements NettyRequestProcessor {
    ...
    public RemotingCommand processRequest(final ChannelHandlerContext ctx, RemotingCommand request)
        ...
        //todo @Toby 【pop消费-概率性消费到重试消息随机数】
        int randomQ = random.nextInt(100);

        //todo @Toby 【pop消费-复活队列取值逻辑】
        int reviveQid;
        if (requestHeader.isOrder()) {
        ...
        getMessageFuture.thenApply(restNum -> {
            if (!getMessageResult.getMessageBufferList().isEmpty()) {
                ...
                if (restNum > 0) {
                    ...
                    //todo @Toby 若未拉满（restNum > 0），通知长轮询服务有新消息时唤醒
                    popLongPollingService.notifyMessageArriving(
    ...
    private CompletableFuture<Long> popMsgFromQueue(String topic, String attemptId, boolean isRetry, GetMessageResult getMessageResult,
        ...
        //todo @Toby 【pop消费-查询消息-加锁处理】锁key=topic_group_queueId
        String lockKey =
        ...
        try {
            ...
            if (isOrder) {
                //todo @Toby 【pop消费-顺序-1】checkBlock判断是否阻塞
            ...
            .thenCompose(result -> {
                ...
                if (!result.getMessageMapedList().isEmpty()) {
                    ...
                    if (isOrder) {
                        ...
                        //todo @Toby 【pop消费-查询消息结果后-追加到checkpoint中】拉取的这批消息将生成一个 CheckPoint，存入内存和磁盘
    ...
    private long getPopOffset(String topic, String group, int queueId, int initMode, boolean init, String lockKey,
        ...
        //todo @Toby 【pop消费-查询消息】从ck里获取偏移量，原理说明如下:
        //1)每次拉取到偏移量后，会在appendCheckPoint的putCkToStore里把ck的偏移量存入commitOffsets中，类似提交偏移量
        //2)所以释放锁后下次再次获取消息时，会从commitOffsets中获取偏移量，如果ck的偏移量大于commitOffsets的偏移量，则返回ck的偏移量，否则返回commitOffsets的偏移量
    ...
    public final MessageExtBrokerInner buildCkMsg(final PopCheckPoint ck, final int reviveQid) {
        ...
        //todo @Toby 【pop消费-putCkToStore】存入revice的topic中，队列id=reviveQid，tag=ck,
    ...
    private boolean appendCheckPoint(final PopMessageRequestHeader requestHeader,
        ...
        //存储计算每个消息的偏移量，值=偏移量-第一个消息的offset
        ...
        //todo @Toby 【pop消费-appendCheckPoint】存入store里，即磁盘中
