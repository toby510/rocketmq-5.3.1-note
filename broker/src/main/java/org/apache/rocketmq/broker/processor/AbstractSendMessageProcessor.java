...
public abstract class AbstractSendMessageProcessor implements NettyRequestProcessor {
    ...
    protected RemotingCommand consumerSendMsgBack(final ChannelHandlerContext ctx, final RemotingCommand request)
        ...
        //todo @Toby【消费失败重试-broker-1】投递到新的topic="%DLQ%"+consumerGroup
        String newTopic = MixAll.getRetryTopic(requestHeader.getGroup());
        //todo @Toby 随机选择一个队列投递
        ...
        //todo @Toby【消费失败重试-broker-2】从重试消息的commitLogOffset物理偏移量获取消息体数据
        ...
        if (null == retryTopic) {
            //todo @Toby 这里比较关键：会把原来的topic设置到消息的属性中，方便后续发送到重试队列时使用
        ...
        boolean isDLQ = false;
        //todo @Toby【消费失败重试-broker-3】如果超过重试次数，则放到死信队列topic="%DLQ%"+consumerGroup
        ...
        //todo @Toby 创建一个新的消息，将重试的消息复制到新的消息中
        ...
        //todo @Toby【消费失败重试-broker-4】新的topic写入commitLog文件
