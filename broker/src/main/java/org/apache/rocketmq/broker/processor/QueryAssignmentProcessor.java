...
public class QueryAssignmentProcessor implements NettyRequestProcessor {
            ...
            case CLUSTERING: {
                ...
                try {
                    ...
                    if (setMessageRequestModeRequestBody != null && setMessageRequestModeRequestBody.getMode() == MessageRequestMode.POP) {
                        //todo @Toby 【pop消费-负载均衡策略-1】allocate4Pop
    ...
    public List<MessageQueue> allocate4Pop(AllocateMessageQueueStrategy allocateMessageQueueStrategy,
        ...
        //todo @Toby 【pop消费-负载均衡策略-broker-1】满足此条件，让当前消费者可见所有队列
        if (popShareQueueNum <= 0 || popShareQueueNum >= cidAll.size() - 1) {
            ...
            for (MessageQueue mq : mqAll) {
                ...
                //todo @Toby 【pop消费-负载均衡策略-broker-2】创建新队列对象，将 queueId 设为 -1（特殊标记，表明是 Pop 模式的全量可见队列）
            ...
            if (cidAll.size() <= mqAll.size()) {
                //todo @Toby 【pop消费-负载均衡策略-broker-3】用基础算法获取自身分配的队列
                ...
                if (index >= 0) {
                    //todo @Toby 【pop消费-负载均衡策略-broker-4】共享后续 popShareQueueNum 个消费者的队列,在队列充足时，通过 “基础分配 + 有限共享” 平衡灵活性与负载：既避免单个消费者处理过多队列（全量分配的极端情况），又允许一定程度的共享（提升消息获取效率）。
