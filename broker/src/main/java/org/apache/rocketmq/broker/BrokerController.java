...
public class BrokerController {
    ...
    protected void initializeBrokerScheduledTasks() {
        ...
        // todo @Toby【提交偏移量-持久化-1】每隔5s,定时任务持久化入口
        this.scheduledExecutorService.scheduleAtFixedRate(new Runnable() {
            @Override
            public void run() {
                try {
    ...
    public boolean initialize() throws CloneNotSupportedException {
        //todo @Toby 【broker-1-加载配置文件】topic配置文件、消费者消费偏移量配置文件、订阅分组配置文件、消费者过滤配置文件
        boolean result = this.initializeMetadata();
        if (!result) {
            return false;
        }
        //todo @Toby 【broker-1-实例化消息存储类DefaultMessageStore】MessageStore消息存储核心组件
    ...
    public boolean recoverAndInitService() throws CloneNotSupportedException {
        ...
        /* todo @Toby 【broker-2-加载消息存储的相关文件】
         * 如果上一步加载配置全部成功
        ...
        //todo @Toby 【broker-2-看似阶梯锐变的时间队列timerMessageStore，具体未知】
        if (messageStoreConfig.isTimerWheelEnable()) {
        ...
        if (result) {
           ...
           //todo @Toby 【broker-2-创建各种执行器线程池】
            initializeResources();

            //todo @Toby 【broker-2-注册处理器】
            registerProcessor();

            //todo @Toby 【broker-2-启动一一系列定时周期任务】
            initializeScheduledTasks();

            //todo @Toby 【broker-2-启动一初始化事务消息相关服务】
            initialTransaction();
            //todo @Toby 【broker-2-初始化权限相关服务】
            initialAcl();
           //todo @Toby 【broker-2-初始化RPC调用的钩子函数】
            initialRpcHooks();
            //todo @Toby 【broker-2-初始化netty的一系列pipeline任务】
    ...
    public void start() throws Exception {
        ...
        //todo @Toby 【broker-2-启动基础服务，启动各种服务和组件，具体进入看】
        startBasicService();


        //todo @Toby 【broker-2-注册当前broker信息到所有的nameserver】
        ...
        //todo @Toby 【broker-2-每隔30s向所有的nameServer进行一次注册broker信息】这个定时任务就是Broker向nameserver发送的心跳包的定时任务，包括topic名、读、写队列个数、队列权限、是否有序等信息。
