...
public interface MappedFile {
    ...
    /**
     * Flushes the data in cache to disk immediately.todo @Toby 刷新时真正刷磁盘，写入磁盘，即将PageCache中的数据强制写入磁盘（commitLog 文件），确保消息持久化
     *
     ...
     * todo @Toby commit：是指消息被写入 PageCache（内存缓冲区），并返回成功响应给生产者，此时消息并未真正持久化到磁盘，而是依赖操作系统的 PageCache 机制后续异步刷盘
     *
