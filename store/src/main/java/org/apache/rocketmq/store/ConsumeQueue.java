...
public class ConsumeQueue implements ConsumeQueueInterface, FileQueueLifeCycle {
    ...
    public ConsumeQueue(
        ...
        //ConsumeQueue文件路径：this.storePath+topic+queueId
        String queueDir = this.storePath
            + File.separator + topic
            + File.separator + queueId;

        //mappedFileSize,默认30M
        this.mappedFileQueue = new MappedFileQueue(queueDir, mappedFileSize, null);
        //todo @Toby 【broker-producer-15】consumeQueue，20个字节,写入在后面的逻辑
    ...
    public void putMessagePositionInfoWrapper(DispatchRequest request) {
        ...
        for (int i = 0; i < maxRetries && canWrite; i++) {
            ...
            //todo @Toby 【broker-producer-17】consumeQueue写入数据入口
    ...
    private boolean putMessagePositionInfo(final long offset, final int size, final long tagsCode,
        ...
        //todo @Toby 【broker-producer-17】consumeQueue写入数据代码位置
        //offset=physicOffset，commitLog的物理偏移量
        this.byteBufferIndex.putLong(offset);
        //size=消息物理大小，长度
        this.byteBufferIndex.putInt(size);
        //tagsCode=对应tag，实际用途未了解
        this.byteBufferIndex.putLong(tagsCode);

        final long expectLogicOffset = cqOffset * CQ_STORE_UNIT_SIZE;

        //mappedFile也类似commitLog，也是通过mappedFileQueue分多个mappedFile，只是每个mappedFile只有20个字节大小，而commitLog默认1G
