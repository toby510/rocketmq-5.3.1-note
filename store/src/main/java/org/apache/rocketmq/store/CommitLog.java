...
public class CommitLog implements Swappable {
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessage(final MessageExtBrokerInner msg) {
        ...
        //todo @Toby 【broker-producer-6】mappedFileQueue获取最近的一个MappedFile
        ...
        if (mappedFile == null) {
            ...
            //todo fileFromOffset文件名：第一条消息的偏移量为文件名，小于20位用0补齐。这里就是拿此mappedFile的文件名，即首偏移量
        ...
        try {
            ...
            //todo @Toby 【broker-producer-7】解析消息存储结构
            ...
            try {
                ...
                if (null == mappedFile || mappedFile.isFull()) {
                    ...
                    //todo @Toby 【broker-producer-8】重新创建一个从0开始的新的mappedFile（mappedFiles为空时）或者基于上一个mappedFile的偏移量创建一个新的mappedFile
                ...
                //todo @Toby 【broker-producer-11】当前mappedFile追加消息
            ...
            //todo @Toby 【broker-producer-18】追加成功，则更新consumeQueueStore的offset，具体待了解
            if (AppendMessageStatus.PUT_OK.equals(result.getStatus())) {
        ...
        //todo @Toby 【broker-producer-19】commitLog刷盘处理
    ...
    public CompletableFuture<PutMessageResult> asyncPutMessages(final MessageExtBatch messageExtBatch) {
        ...
        //ToDo @Toby 【broker-producer-17】看似主从高可用相关，具体待看
        if (needHandleHA && this.defaultMessageStore.getBrokerConfig().isEnableControllerMode()) {
            if (this.defaultMessageStore.getHaService().inSyncReplicasNums(currOffset) < this.defaultMessageStore.getMessageStoreConfig().getMinInSyncReplicas()) {
                return CompletableFuture.completedFuture(new PutMessageResult(PutMessageStatus.IN_SYNC_REPLICAS_NOT_ENOUGH, null));
            }
    ...
    private CompletableFuture<PutMessageResult> handleDiskFlushAndHA(PutMessageResult putMessageResult,
        ...
        //todo @Toby 【broker-producer-20】commitLog刷盘,提交刷盘请求GroupCommitRequest
    ...
    private CompletableFuture<PutMessageStatus> handleHA(AppendMessageResult result, PutMessageResult putMessageResult,
        ...
        //todo @Toby 【broker-producer-20】handleHA,提交请求GroupCommitRequest
    ...
    class CommitRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            ...
            while (!this.isStopped()) {
                //todo @Toby 【broker-producer-26】commitLog异步commit代码
                //todo 但是可以看到它会每隔200ms将页数设置为0，强制提交一次，这其实就是近乎实时的执行 commit。这样能尽可能保证写缓冲区的数据及时提交到 FileChannel 中，避免数据丢失。如果在高并发情况下，想提高commit性能，可以调整 commitCommitLogThoroughInterval 参数，比如每 10s 强制 commit 一次。
                ...
                try {
                    /*
                    --------------------------------------commit和flush对比总结 --------------------------------------
                    操作	            数据位置	           持久化保证	           性能	         适用场景
                    Commit	        PageCache（内存）   可能丢失（依赖后续刷盘）   	高（微秒级）	 允许少量丢失的高吞吐场景
                    Flush（同步）	磁盘	              强一致（不丢失）          	低（毫秒级）	 金融、交易等关键业务
                    Flush（异步）	磁盘（延迟刷）	  可能丢失少量数据	            较高	         普通业务
                     --------------------------------------commit和flush对比总结 --------------------------------------
                    */
                    //todo @Toby 【broker-producer-26.1】commitLog异步commit代码位置（commit和flush区别见MappedFile类的方法备注）
                    boolean result = CommitLog.this.mappedFileQueue.commit(commitDataLeastPages);
                    long end = System.currentTimeMillis();
                    if (!result) {
                        //todo @Toby 【broker-producer-26.2】commit 数据之后，就会唤醒 FlushRealTimeService 来执行一次 flush，保证数据刷到磁盘。
    ...
    class FlushRealTimeService extends FlushCommitLogService {
        ...
        public void run() {
            CommitLog.log.info(this.getServiceName() + " service started");
            //todo @Toby 【broker-producer-25】commitLog异步刷盘代码
            while (!this.isStopped()) {
                //// 定时刷盘 CommitLog，默认开启
                boolean flushCommitLogTimed = CommitLog.this.defaultMessageStore.getMessageStoreConfig().isFlushCommitLogTimed();
                //todo @Toby 【broker-producer-25.1】commitLog异步刷盘代码,间隔时间，默认 500ms
                ...
                try {
                    ...
                    long begin = System.currentTimeMillis();
                    //todo @Toby 【broker-producer-25.2】commitLog异步刷盘代码，flush入口
    ...
    class GroupCommitService extends FlushCommitLogService {
            ...
            if (!this.requestsRead.isEmpty()) {
                for (GroupCommitRequest req : this.requestsRead) {
                    ...
                    for (int i = 0; i < 1000 && !flushOK; i++) {
                        //todo @Toby 【broker-producer-24】commitLog刷盘代码位置
        ...
        public void run() {
            CommitLog.log.info(this.getServiceName() + " service started");
            //todo @Toby 【broker-producer-23】commitLog刷盘线程
            while (!this.isStopped()) {
                try {
    ...
    class DefaultFlushManager implements FlushManager {
        ...
        public CompletableFuture<PutMessageStatus> handleDiskFlush(AppendMessageResult result, MessageExt messageExt) {
            // Synchronization flush
            //todo @Toby 【broker-producer-21】commitLog刷盘,提交同步刷盘请求GroupCommitRequest
            if (FlushDiskType.SYNC_FLUSH == CommitLog.this.defaultMessageStore.getMessageStoreConfig().getFlushDiskType()) {
                final GroupCommitService service = (GroupCommitService) this.flushCommitLogService;
                if (messageExt.isWaitStoreMsgOK()) {
                    GroupCommitRequest request = new GroupCommitRequest(result.getWroteOffset() + result.getWroteBytes(), CommitLog.this.defaultMessageStore.getMessageStoreConfig().getSyncFlushTimeout());
            ...
            // Asynchronous flush
                ...
                //todo @Toby 【broker-producer-22】commitLog刷盘,提交异步刷盘请求GroupCommitRequest
                //todo 如果启用了 TransientStorePool，则由 CommitRealTimeService 先执行 commit，再由 FlushRealTimeService 执行 flush
                if (!CommitLog.this.defaultMessageStore.isTransientStorePoolEnable()) {
                    flushCommitLogService.wakeup();
                } else {
                    commitRealTimeService.wakeup();
