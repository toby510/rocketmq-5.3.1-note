...
public class AllocateMessageQueueConsistentHash extends AbstractAllocateMessageQueueStrategy {
    ...
    public List<MessageQueue> allocate(String consumerGroup, String currentCID, List<MessageQueue> mqAll,
        ...
        //todo @Toby 【队列分配策略-一致性hash-1】将消费者节点列表传入并添加到环中，支持自定义hash，默认使用MD5
        ...
        for (MessageQueue mq : mqAll) {
            ...
            //todo @Toby 【队列分配策略-一致性hash-4】这里计算每个消息队列的hash值，获取环中比它大的第一个节点（利用TreeMap.tailMap视图:返回比fromKey大的所有map，且会跟原map同步变化）
            //todo @Toby 【队列分配策略-一致性hash-5】命中了当前消费者节点虚拟节点对应的物理节点，则加入结果列表
