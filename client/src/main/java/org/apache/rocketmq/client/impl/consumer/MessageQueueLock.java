...
public class MessageQueueLock {
    ...
    //todo @Toby 【顺序消费】 获取锁对象
    // fetchLockObject方法内部如果没有这个对象则会创建一个（内部可以保证一个MessageQueue不会生成两个Object）。然后这个objLock用于实现消费逻辑的加锁，这里可以看出，如果MessageQueue和Object的关系是一对一的话，那么消费就会是，一个队列只有一个线程进行消费。
  Object fetchLockObject(final MessageQueue mq, final int shardingKeyIndex) {
