    ...
    public void processConsumeResult(
        ...
        switch (this.defaultMQPushConsumer.getMessageModel()) {
            ...
            case CLUSTERING:
                ...
                //todo @Toby 【consumer消费失败-处理结果】根据ackIndex处理结果，如果status=CONSUME_SUCCESS,则不会执行下面这个for循环
                for (int i = ackIndex + 1; i < consumeRequest.getMsgs().size(); i++) {
                    MessageExt msg = consumeRequest.getMsgs().get(i);
                    ...
                    //todo @Toby 【consumer消费失败重试入口-cosumer】
        ...
        //todo @Toby 【consumer】不管消息消费成功还是失败，将消费过的消息从ProcessQueue中删除
        long offset = consumeRequest.getProcessQueue().removeMessage(consumeRequest.getMsgs());
        if (offset >= 0 && !consumeRequest.getProcessQueue().isDropped()) {

            //todo @Toby【consumer】更新偏移量方法
            //todo @Toby 不管成功失败，都会提交偏移量，偏移量=本次拉取的批量消息中最大的那一个
            // todo @Toby【提交偏移量-计算从何处提交-3】提交偏移量入口，集群模式为RemoteBrokerOffsetStore
        ...
        public void run() {
            ...
            try {
                if (msgs != null && !msgs.isEmpty()) {
                    //todo @Toby【consumer】调用业务listener前，塞入开始执行时间，有定时任务扫描是否超时，超时放入重试队列里
                    for (MessageExt msg : msgs) {
                        MessageAccessor.setConsumeStartTimeStamp(msg, String.valueOf(System.currentTimeMillis()));
                    }
                }
                //todo @Toby【consumer】消费消息，调用业务代码入口 listener.consumeMessage
            ...
            //todo @Toby 如果某个pq被丢弃了，则不再提交偏移量
            if (!processQueue.isDropped()) {
                //todo @Toby【consumer-处理消费结果】根据业务结果判断是否需要重试
