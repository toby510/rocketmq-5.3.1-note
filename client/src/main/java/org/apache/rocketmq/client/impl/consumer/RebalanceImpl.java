        ...
        if (findBrokerResult != null) {
            ...
            try {
                ...
                for (MessageQueue mmqq : lockedMq) {
                    ...
                    //todo @Toby 【顺序消费】mq如果加锁成功，则会把ProcessQueue也进行加锁，这个会在消费的还是进行判断是否有加锁。
                    if (processQueue != null) {
        ...
        while (it.hasNext()) {
            ...
            if (findBrokerResult != null) {
                ...
                try {
                    ...
                    for (MessageQueue mq : mqs) {
                        //todo @Toby 【顺序消费】只有经过重平衡后processQueueTable才有值，所以到这里说明processQueueTable已经分配好了。之所以定时锁定，个人猜测是为了续期或者避免进程挂掉导致的顺序问题
                        ProcessQueue processQueue = this.processQueueTable.get(mq);
                        if (processQueue != null) {
        ...
        if (subTable != null) {
            for (final Map.Entry<String, SubscriptionData> entry : subTable.entrySet()) {
                ...
                try {
                    ...
                    //todo @Toby 【pop消费-负载均衡策略-consumer-1】tryQueryAssignment请求broker获取分配结果
    ...
    private boolean tryQueryAssignment(String topic) {
        ...
        while (retryTimes++ < TIMEOUT_CHECK_TIMES) {
            try {
                ...
                //todo @Toby 【pop消费-负载均衡策略-consumer-2】queryAssignment请求broker获取分配结果
    ...
    private boolean updateProcessQueueTableInRebalance(final String topic, final Set<MessageQueue> mqSet,
        ...
        while (it.hasNext()) {
            ...
            if (mq.getTopic().equals(topic)) {
                //todo @Toby 【consumer-重平衡】processQueueTable之前分配的mq不在本次分配的mq中，则移除掉
        ...
        //todo @Toby 【consumer-重平衡】移除不属于我的mq，会将processQueue的dropped设置为true，消费时会丢弃
        for (Entry<MessageQueue, ProcessQueue> entry : removeQueueMap.entrySet()) {
            MessageQueue mq = entry.getKey();
            ProcessQueue pq = entry.getValue();

            //todo @Toby 【consumer-重平衡】removeUnnecessaryMessageQueue:dropped设置为true&&移除偏离&&释放锁&&提交偏移量
        ...
        for (MessageQueue mq : mqSet) {
            if (!this.processQueueTable.containsKey(mq)) {
                //todo @Toby 【顺序消费-第1把锁-broker锁】重平衡后即使分配到了分区，但是如果获取不到锁，也无法消费
                ...
                //todo @Toby 【顺序消费】一定是要向broker申请messageQueue锁成功之后，才能构建processQueue并且加入到processQueueTable
