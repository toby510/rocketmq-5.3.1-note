...
public class DefaultMQPushConsumerImpl implements MQConsumerInner {
    ...
    public void pullMessage(final PullRequest pullRequest) {
        ...
        PullCallback pullCallback = new PullCallback() {
            ...
            public void onSuccess(PullResult pullResult) {
                if (pullResult != null) {
                    ...
                    //todo @Toby 【consumer-5.3.11】收到broker回调，再次过滤tag(第三次过滤tag:consuemr过滤)
                    ...
                    switch (pullResult.getPullStatus()) {
                        case FOUND:
                            ...
                            if (pullResult.getMsgFoundList() == null || pullResult.getMsgFoundList().isEmpty()) {
                                ...
                                //todo @Toby 【consumer-5.3.12】收到broker返回的拉取消息，提交到消费线程池里，异步消费
        ...
        try {
            ...
            //todo @Toby 【consumer-5.3.2】pullAPIWrapper发送netty的api请求，拉取消息，拉取后通过pullCallback回调。核心入参： pullRequest.getMessageQueue()，subExpression表达式
    ...
    void popMessage(final PopRequest popRequest) {
                ...
                switch (popResult.getPopStatus()) {
                    case FOUND:
                        ...
                        if (popResult.getMsgFoundList() == null || popResult.getMsgFoundList().isEmpty()) {
                            ...
                            //todo @Toby 【pop消费-异步拉取消息】 拉取结果回调，将结果提交到消费线程池进行消费，submitPopConsumeRequest
        ...
        try {
            ...
            //todo @Toby 【pop消费-异步拉取消息】 默认一分钟可见时间
            ...
            //todo @Toby 【pop消费-异步拉取消息】 this.pullAPIWrapper.popAsync，默认一分钟可见时间
    ...
    public synchronized void start() throws MQClientException {
        switch (this.serviceState) {
            case CREATE_JUST:
                ...
                //todo @Toby 【consumer-4.1】消费核心逻辑---复制订阅关系---订阅业务topic时还订阅了重试topic，名称=%RETRY%+消费组名称
                //todo MixAll.getRetryTopic(this.defaultMQPushConsumer.getConsumerGroup())
                this.copySubscription();

                if (this.defaultMQPushConsumer.getMessageModel() == MessageModel.CLUSTERING) {
                    this.defaultMQPushConsumer.changeInstanceNameToPID();
                }
                //todo @Toby 【consumer-4.2】查询或创建客户端实例（核心）,负责跟broker通信，非常核心： this.factoryTable.putIfAbsent(clientId, instance)
                this.mQClientFactory = MQClientManager.getInstance().getOrCreateMQClientInstance(this.defaultMQPushConsumer, this.rpcHook);

                //todo @Toby 【consumer-4.3】复杂均衡组件，设置消费群组、消费模式、队列分配策略以及mQClientFactory
                ...
                //todo @Toby 【consumer-4.4】设置偏移量存储组件，如果是广播模式则存储在客户端本地，如果是集群模式则存储在broker端
                ...
                //todo @Toby 【consumer-4.5】加载偏移量，集群模式为空实现
                this.offsetStore.load();


                //todo @Toby 【consumer-4.6】根据是否顺序消息，来选择对应的service，顺序则=ConsumeMessageOrderlyService，否则=ConsumeMessageConcurrentlyService
                ...
                //todo @Toby 【consumer-4.7】启动消息消费服务（核心），即consumeMessageService，里面是调度线程每间隔20s，向broker发起队列锁写入请求，即 ConsumeMessageOrderlyService.this.lockMQPeriodically();
                this.consumeMessageService.start();
                // POPTODO
                this.consumeMessagePopService.start();
                //todo @Toby 【consumer-4.8】注册消费者：this.consumerTable.putIfAbsent(group, consumer)
                ...
                //todo @Toby 【consumer-5】拉取消息+重平衡（核心核心核心）
        ...
        try {
            ...
            switch (this.defaultMQPushConsumer.getMessageModel()) {
                case BROADCASTING:
                    break;
                case CLUSTERING:
                    //todo @Toby 【consumer-4.2】构建重试topic，格式：%RETRY%+消费组名，构建完后，放到此消费者的复杂均衡组件的订阅关系里，即：this.rebalanceImpl.getSubscriptionInner()
                    //todo 进而得知：此消费者除了订阅业务topic，还同时订阅了retryTopic，且订阅表达式为 *（SUB_ALL），表示消费该 Topic 下所有消息。
